...
describe('AuthenticationPayloadCreator', () => {
  ...
  describe('create', () => {
    let providerSpy: jest.Spied<any>;
    let signatureProviderSpy: jest.Spied<any>;

    beforeEach(() => {
      providerSpy = jest
        .spyOn(authenticationPayloadCreator as any, 'provider')
        .mockResolvedValue(credentials);
      signatureProviderSpy = jest
        .spyOn(
          (authenticationPayloadCreator as any).signature,
          'credentialProvider',
        )
        .mockResolvedValue(credentials);
    });
    ...
    it('should return correct authentication payload', async () => {
      ...
      expect(payload).toHaveProperty(
        'x-amz-credential',
        `${credentials.accessKeyId}/20210101/${region}/kafka-cluster/aws4_request`,
      );
      ...
      expect(payload).toHaveProperty(
        'x-amz-signature',
        '2335694f37dc794a44059b4ca0662efed0db99586b14688eaa89d771b8a1e4b3',
      );
    });
  });
});
